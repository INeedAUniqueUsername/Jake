...
public class GameArea extends JPanel implements KeyListener, Runnable {
	...
	private Player_Snake_Head[] playerz;
...
/*	Food[] foodzlist;*/
	...
	public void reset()
	{
		...
		timer = 0;
		score = 0;
		playerz = new Player_Snake_Head[players];
		for(int i = 0; i < playerz.length; i++)
		{
			playerz[i] = new Player_Snake_Head(randomWidth(), randomHeight(), "RIGHT", 1, 20, 150);
		}
		enemiez = new Enemy_Snake_Head[enemyNumberStarting];
		for(int i = 0; i < enemiez.length; i++)
		{
			enemyTotal = enemyTotal + 1;
			enemiez[0] = new Enemy_Snake_Head(randomWidth(), randomHeight(), "LEFT", 1, 2, 300, enemyTotal); //1600, 160
		}
...
/*			reset = false;
		...
		{	
			...
			for(Player_Snake_Head p: playerz)
			{
				if(!(p == null))
				{
					...
					p.move();
					p.draw(window);
					for(Player_Snake_Body pb: p.bodies)
					{
						if(!(pb == null))
						{
							pb.draw(window);
			...
			for(Enemy_Snake_Head e: enemiez)
			{
				if(!(e == null))
				{
					...
					if(e.targetType == "")
					{
						...
						Player_Snake_Head nearestPlayer = null;
						for(int i_p = 0; i_p < playerz.length; i_p++)
						{
							Player_Snake_Head p = playerz[i_p];
							if(!(p == null))
							{
					...
					for(Player_Snake_Head p: playerz)
					{
						if(!(p == null))
						{
							...
							if(f.xPos == p.xPos && f.yPos == p.yPos && !(p.abilityMode.equals("TUNNELLING")))
							{
								...
								enemyTotal = enemyTotal + 1;
								enemiez[enemiez.length-1] = new Enemy_Snake_Head(randomWidth(), randomHeight(), "DOWN", 1, score + 2, 300 - (25*score), enemyTotal);
							}
						}
					}
					//Player-Food Collisions
						...
						if(!(e == null))
						{
							...
							{	
								...
								for(int i = 0; i < enemiez.length; i++)
								{
									...
									{
										enemyTotal = enemyTotal + 1;
										enemiez[i] = new Enemy_Snake_Head(randomWidth(), randomHeight(), "DOWN", 1, score + 2, 300 - (25*i), enemyTotal);
									}
								}
							}
						}
			...
			{
				Player_Snake_Head p = playerz[ip];
				if(!(p == null))
				{
					//Player bites wall
			...
			for(int ip = 0; ip < playerz.length; ip++)
			{
				...
				if(!(p == null))
				{
					...
					{
						Player_Snake_Head other_p = playerz[i_other_p];
						...
						if(!(playerz[i_other_p] == null) && !(playerz[ip] == playerz[i_other_p]))
						{
							...
							for(Player_Snake_Body other_pb: other_p.bodies)
							{
...
/*			//System.out.println(foodz[0].xPos);
	...
	public void enemyTargeting(String enemyType, Enemy_Snake_Head e)
	{
		...
		if(e.targetType.equals("PLAYER"))
		{
			...
			if(!(e.targetPlayer == null))
			{
				...
				{
					Player_Snake_Body targetPlayerTail = e.targetPlayer.bodies[0];
	...
	public void targetPlayerHead(Enemy_Snake_Head e)
	{
		e.horizontalDiff = Math.abs((e.xPos + (e.width/2)) - (e.targetPlayer.xPos + (e.targetPlayer.width/2)));
		e.verticalDiff = Math.abs((e.yPos + (e.height/2)) - (e.targetPlayer.yPos + (e.targetPlayer.height/2)));
		e.aboveTarget = e.yPos + e.height/2 < e.targetPlayer.yPos;
	...
	public void targetPlayerBody(Enemy_Snake_Head e, Player_Snake_Body pb)
	{
	...
	public void keyPressed(KeyEvent e) {
		...
		if(!(playerz[0] == null))
		{
			Player_Snake_Head player1 = playerz[0];
		...
		if(!(playerz[1] == null))
		{
			Player_Snake_Head player2 = playerz[1];
